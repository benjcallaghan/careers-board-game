class Player {
  ...
  moveOnce() {
    ...
    console.log(
      `${this.#id} passed space #${nextSpace.id}.${nextSpace.className}.`,
    );
  }
...
goalsForm.addEventListener("input", (e) => {
  totalOutput.value =
    happinessInput.valueAsNumber +
    fameInput.valueAsNumber +
    Math.floor(fortuneInput.valueAsNumber / 10000);
...
goalsForm.addEventListener("submit", (e) => {
  if (!totalOutput.validity.valid) {
    e.preventDefault();
    totalOutput.nextElementSibling.textContent =
      "The total points must add up to 60.";
  }
});
